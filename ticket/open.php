...
if (isset($_SESSION['user'])) {
	...
	$check_ticket = $db->query("SELECT * FROM tickets WHERE id = '$post_target' AND user = '$sess_username'");

	$data_ticket = $check_ticket->fetch_array(MYSQLI_ASSOC);
	$check_reply = $db->query("SELECT * FROM tickets_message WHERE ticket_id = '$post_target' AND sender = 'Admin'");
	if ($check_ticket->num_rows == 0) {
		header("Location: ".$site_config['base_url']."ticket/new.php");
	} else {
		$db->query("UPDATE tickets SET seen_user = '1' WHERE id = '$post_target'");
		if (isset($_POST['submit'])) {
			...
			if ($data_ticket['status'] == "Closed") {
				$msg_type = "error";
			    $msg_content = '<b>Gagal!</b> Tiket telah ditutup.';
				...
				$insert_ticket = $db->query("INSERT INTO tickets_message (ticket_id, sender, user, message, datetime) VALUES ('$post_target', 'Member', '$sess_username', '$post_message', '$last_update')");
				$update_ticket = $db->query("UPDATE tickets SET last_update = '$last_update' WHERE id = '$post_target'");
				if ($check_reply->num_rows > 0) {
					$db->query("UPDATE tickets SET status = 'Waiting', seen_admin = '0' WHERE id = '$post_target'");
				}
				if ($insert_ticket == TRUE) {
					$msg_type = "success";
					$msg_content = "<b>Berhasil!</b> Pesan terkirim.";
				} else {
					$msg_type = "error";
...
?>
                <div class="row">
                    <div class="col-md-8 offset-2">
                    	<a href="<?php echo $site_config['base_url']; ?>ticket/new.php" class="btn btn-secondary btn-bordred m-b-20"><i class="fa fa-reply"></i> Kembali</a>
                        <div class="card-box">
