...
if (isset($_SESSION['user'])) {
	...
	if ($check_user->num_rows == 0) {
	    ...
	    if (isset($_POST['add'])) {
			...
			$post_id = $db->real_escape_string($_GET['servicecat_id']);
			$post_name = $db->real_escape_string(stripslashes(strip_tags(htmlspecialchars($_POST['name'], ENT_QUOTES))));
			$post_code = $db->real_escape_string(stripslashes(strip_tags(htmlspecialchars($_POST['code'], ENT_QUOTES))));
			...
			if (empty($post_name) || empty($post_code)) {
				...
				$update_category = $db->query("UPDATE service_cat SET name = '$post_name', code = '$post_code' WHERE id = '$post_id'");
			...
			$post_id = $db->real_escape_string($_GET['servicecat_id']);
			$checkdb_news = $db->query("SELECT * FROM service_cat WHERE id = '$post_id'");
			if ($checkdb_news->num_rows == 0) {
				$msg_type = "error";
				$msg_content = "<b>Gagal!</b> Kategori tidak ditemukan.";
			} else {
				$delete_news = $db->query("DELETE FROM service_cat WHERE id = '$post_id'");
				if ($delete_news == TRUE) {
					$msg_type = "success";
                ...
                <div class="card-box">
					...
					<div class="table-responsive">
                        <table id="datatable" class="table table-bordered table-striped">
							...
							<tbody>
							<?php
							$data_category = $db->query("SELECT * FROM service_cat ORDER BY id DESC");
