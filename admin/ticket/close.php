			...
			if ($check_ticket->num_rows == 0) {
				...
				if (isset($_POST['submit'])) {
					if ($data_ticket['status'] == "Closed") {
						$msg_type = "error";
						$msg_content = "<b>Gagal!</b> Tiket sudah ditutup.";
					} else {
						$update_ticket = $db->query("UPDATE tickets SET status = 'Closed' WHERE id = '$post_target'");
						if ($update_ticket == TRUE) {
							$msg_type = "success";
							$msg_content = "<b>Berhasil:</b> Ticket ditutup.";
						} else {
							$msg_type = "error";
...
?>
                ...
                <div class="row">
                    <div class="offset-lg-2 col-lg-8">
                        <div class="card-box">
                            <h4 class="m-t-0 text-uppercase header-title"><i class="fa fa-times"></i> Tutup Tiket</h4><hr>
							...
							<form class="form-horizontal" role="form" method="POST">
								...
								<div class="form-group row">
									<div class="col-md-8 offset-2">
										...
										<button type="cancel" class="btn btn-custom btn-bordred" name="submit"><i class="fa fa-send"></i> Tutup </button>	
						     	    </div>
								</div>
							</form>
						</div>
